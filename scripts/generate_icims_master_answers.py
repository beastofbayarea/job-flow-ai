...
ICIMS_QUEUE_PATH = Path("data/application-queues/icims-job-search-2026-08-08.json")
...
DATA_MD = Path(r"c:\Users\Nagarro\Downloads\job-flow-ai\data\icims_master_answers.md")
ARTIFACT_MD = Path(r"C:\Users\Nagarro\.gemini\antigravity-ide\brain\ffb15041-044d-471d-8f91-40eaee4b2ad7\icims_master_answers.md")
...
def run():
    jobs = json.loads(ICIMS_QUEUE_PATH.read_text(encoding="utf-8"))
    email_pool = json.loads(POOL_PATH.read_text(encoding="utf-8"))
    
    random.seed(99) # Deterministic selection
    selected_emails = random.sample(email_pool, len(jobs))
    ...
    for idx, j in enumerate(jobs, 1):
        ...
        copy_filename = f"iCIMS - {clean_company} - {clean_title}.pdf"
        ...
        lines.append(f"* **Application Form URL**: `{raw_url}`")
        lines.append("")
    ...
    print(f"Successfully generated 100% complete field answers for ALL {len(jobs)} iCIMS applications!")
    print(f"Saved to:\n - {DATA_MD}\n - {ARTIFACT_MD}")
