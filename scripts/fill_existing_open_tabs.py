...
def fill_open_tabs_only():
    with sync_playwright() as p:
        print("Connecting to existing Chrome browser on http://localhost:9222...")
        ...
        print(f"Found {len(ashby_pages)} open Ashby application pages in your browser.")
        
        for idx, page in enumerate(ashby_pages, 1):
            url = page.url.lower()
            title = page.title()
            print(f"\n[{idx}/{len(ashby_pages)}] Filling open tab: {title} ({page.url})")
            ...
            try:
                ...
                if file_inputs.count() > 0:
                    try:
                        ...
                        print("  Attached resume: resume-general.pdf")
                ...
                print(f"  Successfully filled {len(fill_res)} fields in tab.")
