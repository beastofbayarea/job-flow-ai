...
def run_playwright_cdp_fill():
    with sync_playwright() as p:
        print("Connecting to Chrome over CDP on http://localhost:9222...")
        ...
        print(f"Total open pages in Chrome: {len(all_pages)}")
        
        ashby_pages = [page for page in all_pages if "jobs.ashbyhq.com" in page.url]
        print(f"Found {len(ashby_pages)} open Ashby application pages.")
        
        for idx, page in enumerate(ashby_pages, 1):
            url = page.url.lower()
            title = page.title()
            print(f"\n[{idx}/{len(ashby_pages)}] Processing: {title} ({page.url})")
            
            # Match company answers
            matched_custom = {}
            for comp_key, ans_dict in ANSWERS_MAP.items():
                if comp_key in url or comp_key in title.lower():
            ...
            try:
                ...
                # Execute JS to fill form fields & custom answers
                ...
                print(f"  Filled {len(fill_res)} field(s) in tab.")
            except Exception as e:
                print(f"  Error filling tab {title}: {e}")
                
if __name__ == "__main__":
    run_playwright_cdp_fill()
