...
def activate_target_in_chrome(target_id):
    try:
        req = urllib.request.Request(f"{ENDPOINT}/json/activate/{target_id}")
        with urllib.request.urlopen(req) as resp:
            resp.read()
    except Exception as e:
        pass

def fill_single_target(target):
    ...
    activate_target_in_chrome(t_id)
    time.sleep(1.0) # Allow tab activation & DOM repaint
    
    print(f"\nFilling open tab: {title} ({url})...")
    ...
    try:
        ws = connect(ws_url, open_timeout=6, close_timeout=1)
        msg_id = 0
        ...
        filled = eval_res.get("result", {}).get("value", {})
        ...
        try:
            ...
            if root_node:
                ...
                if file_node:
                    send_cdp("DOM.setFileInputFiles", {"files": [str(RESUME_PATH)], "nodeId": file_node})
                    print("  Attached resume successfully.")
        except Exception as fe:
            print("  Resume attach note:", fe)

        ws.close()
    except Exception as e:
        print(f"  Error on tab {title}: {e}")
...
def main():
    ...
    print(f"Found {len(ashby_targets)} open Ashby tabs in your browser.")
    
    for t in ashby_targets:
        fill_single_target(t)
