...
def clean_duplicate_tabs_and_fill():
    # 1. Fetch CDP targets
    req = urllib.request.Request(ENDPOINT + "/json/list")
    with urllib.request.urlopen(req) as resp:
        targets = json.load(resp)
        
    print(f"Total open CDP targets in Chrome: {len(targets)}")
    
    seen_urls = {}
    duplicates_closed = 0
    unique_ashby_targets = []
    ...
    for t in targets:
        ...
        if "jobs.ashbyhq.com" in url:
            # Normalize URL to base job application path (ignore query params)
            base_url = url.split("?")[0].rstrip("/")
            if not base_url.endswith("/application"):
                base_url += "/application"
                
            if base_url in seen_urls:
                # Close duplicate tab via CDP
                try:
                    close_req = urllib.request.Request(f"{ENDPOINT}/json/close/{target_id}")
                    with urllib.request.urlopen(close_req) as cr:
                        cr.read()
                    print(f"Closed duplicate tab ({target_id}): {t.get('title')}")
                    duplicates_closed += 1
                except Exception as ce:
                    print(f"Error closing target {target_id}: {ce}")
            else:
                seen_urls[base_url] = t
                unique_ashby_targets.append(t)

    print(f"Closed {duplicates_closed} duplicate tabs. Retained {len(unique_ashby_targets)} unique Ashby application tabs.")

    # 2. Fill each unique open tab via Playwright CDP
    with sync_playwright() as p:
        ...
        for idx, t in enumerate(unique_ashby_targets, 1):
            t_id = t.get("id")
            t_url = t.get("url", "").lower()
            t_title = t.get("title", "")
            
            # Find matching page object
            page = next((pg for pg in all_pages if "jobs.ashbyhq.com" in pg.url and (t_id in pg.url or pg.url.split("?")[0].rstrip("/") in t_url)), None)
            if not page and len(all_pages) >= idx:
                page = all_pages[idx - 1]
                
            if not page:
                print(f"[{idx}] Could not attach Playwright page for {t_title}")
                continue

            print(f"\n[{idx}/{len(unique_ashby_targets)}] Filling single tab: {t_title} ({page.url})")
            
            matched_custom = {}
            for comp_key, ans_dict in ANSWERS_MAP.items():
                if comp_key in t_url or comp_key in t_title.lower():
                    matched_custom = ans_dict
                    break
            ...
            try:
                ...
                if file_inputs.count() > 0:
                    try:
                        ...
                        print(f"  Resume attach note: {fe}")
                
                payload = json.dumps({"values": STANDARD_VALUES, "answers": matched_custom})
                
                fill_res = page.evaluate("""(payload) => {
                ...
                print(f"  Filled {len(fill_res)} fields in tab.")
            except Exception as e:
                print(f"  Error filling tab {t_title}: {e}")

if __name__ == "__main__":
    clean_duplicate_tabs_and_fill()
