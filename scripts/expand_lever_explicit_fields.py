...
def run():
    ...
    # Split text into lines to reliably parse headers
    ...
    for line in lines:
        line_str = line.strip()
        m_head = re.match(r"^###\s*(\d+)\.\s*(.*?)\s*—\s*(.*)$", line_str)
        if m_head:
            if current_entry:
                entries.append(current_entry)
            current_entry = {
                "index": m_head.group(1).strip(),
                "company": m_head.group(2).strip(),
        ...
        if current_entry:
            m_url = re.search(r"\*+\s*URL\*+:\s*`([^`]+)`", line_str)
            if m_url:
                current_entry["url"] = m_url.group(1).strip()
            m_email = re.search(r"\*+\s*Email\*+:\s*`([^`]+)`", line_str)
            if m_email:
                current_entry["email"] = m_email.group(1).strip()
                
    if current_entry:
        entries.append(current_entry)
        
    print(f"Parsed {len(entries)} application entries from {MD_PATH}")
    ...
    print(f"Successfully expanded ALL {len(entries)} applications in {MD_PATH}!")
