...
EXTRACTED_ALL_PATH = Path("data/lever_all_48_extracted_fields.json")
...
def run():
    ...
    with sync_playwright() as p:
        ...
        for idx, j in enumerate(all_jobs, 1):
            ...
            print(f"[{idx}/{len(all_jobs)}] Extracting {company} — {title}...")
            ...
            try:
                ...
                extracted_data[f"{idx}_{company}"] = {
                    "index": idx,
                    "company": company,
                    "title": title,
                    "url": apply_url,
    ...
    EXTRACTED_ALL_PATH.write_text(json.dumps(extracted_data, indent=2, ensure_ascii=False), encoding="utf-8")
    
    # Construct Markdown Master Answers Guide for all 48 Jobs
    md_lines = [
        "# 100% Complete Master Field & Answer Guide for ALL 48 Lever Job Applications",
        "",
    ...
    print(f"\nAll 48 Lever master answers guide generated at {OUT_MD_PATH}")
